...
public class userBean implements Serializable {
    ...
    public void signIn(String uName) {
        ...
        String sql = "SELECT * FROM DB_ADMIN.CUSTOMERS WHERE USERNAME = '" + uName + "'";
        ...
        //If credentials don't match, display an error
        } else {
            fc.addMessage(null, new FacesMessage("Incorrect Username or Password, please try again."));
        }
    }
    ...
    public void signUp() throws IOException, SQLException {
        ...
        String insertUserSQL = "INSERT INTO DB_ADMIN.CUSTOMERS (username, customer_name, password, mobile_number, email) "
                + "values ('" + mainBean.getTempNewUsername() + "', '" + mainBean.getTempNewName() + "',"
                + " '" + mainBean.getTempNewPassword() + "', '" + mainBean.getTempNewMobileNumber() + "',"
                + " '" + mainBean.getTempNewEmail() + "')";

        //Look for inputted username in the database
        String checkUserSQL = "SELECT * FROM DB_ADMIN.CUSTOMERS WHERE username = '" + mainBean.getTempNewUsername() + "'";

        //Check if inputted passwords match and if they aren't empty
        if ((mainBean.getTempNewPassword().equalsIgnoreCase(mainBean.getTempNewPasswordCheck())) && ((!mainBean.getTempNewPassword().equals("")))) {
            Boolean check = false;
            try {
                Statement stment = cnt.createStatement();
                stment.execute(checkUserSQL);
            ...
            if ((check == false) && (!mainBean.getTempNewUsername().equals(""))) {
                try {
                    Statement stment = cnt.createStatement();
                    stment.executeUpdate(insertUserSQL);
        ...
        //If password does not match or is invalid, display error message
            ...
            fc.addMessage(null, new FacesMessage("The password is invalid or does not match, please try again."));
        }
    }
